...
const Submit2 = () => {
    const heroStyle = {
      ...
      useEffect(() => {
        const fetchUserData = async () => {
          try {
            const response = await axios.get('https://gamer-hub-server.onrender.com/loggedIn', { withCredentials: true });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post('https://gamer-hub-server.onrender.com/addReview', {
        user: userid,
        username: username,
        game: appid,
        title: gameData.name,
